...
public class ProjectService {
    ...
    /**
     * Calculate total HR cost of a project based on employee allocation and salary.
     */
    ...
    /**
     * Helper: Calculate full months between two dates, rounding up.
     */
    private int calculateMonthsBetween(Date startDate, Date endDate) {
        ...
        int totalMonths = yearDiff * 12 + monthDiff + 1; // +1 to round up partial month

        return Math.max(totalMonths, 0);
    }

    /**
     * Get all active projects for a department, sorted by a specified field
     */
    public List<Project> getProjectsByDepartment(int departmentId, String sortBy) throws Exception {
        if (sortBy == null || sortBy.isEmpty()) {
            sortBy = "endDate"; // default sorting
        }

        // Delegate to repository
        List<Project> projects = projectRepository.getProjectsByDepartment(departmentId, sortBy);
